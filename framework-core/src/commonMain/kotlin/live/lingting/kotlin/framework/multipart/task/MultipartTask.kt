...
abstract class MultipartTask @JvmOverloads protected constructor(
        ...
        tasks.forEach { task ->
            async.submit {
                try {
                    while (true) {
                        try {
                            task.status = PartTask.Status.RUNNING
                            onPart(task)
                            task.t = null
                            task.status = PartTask.Status.SUCCESSFUL
                            break
                        } catch (t: Throwable) {
                            task.t = t
                            task.status = PartTask.Status.FAILED
    ...
    suspend fun await(timeout: Duration? = null) {
        Await.waitTrue(timeout) { isCompleted }
    }
    ...
    enum class Status {
        ...
        class Serializer : EnumSerializer<Status>(enumValues<Status>())
